...
from graph.t2_e1_strongly_connected_component import depth_first_search, sort_vertices_by_final_times, strongly_connected_components
...
def initGraph3():
    ...
    ef = Edge(e, f)
    de = Edge(d, e)
    e = [ab, bd, dc, ca, bf, fe, ef, de]
    return Graph(v, e, oriented=True)
...
def test_strongly_connected_components():
    graph = initGraph3()
    antecessors, graph = strongly_connected_components(graph)
    assert antecessors == []